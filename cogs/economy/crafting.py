...
for i in craftable_items:
    options.append(
        discord.SelectOption(
            label=i,
            description=items[i.lower()]["description"],
            emoji=items[i.lower()]["emoji"],
            default=(i == selected_item),
        )
    )
...
class craftingview(discord.ui.View):
    ...
    @discord.ui.button(label="Craft", emoji="🛠️", style=discord.ButtonStyle.green, row=1)
    async def craft_callback(
        ...
        user_crafting: CraftingData = await data_manager.get_user_crafting(
            interaction.user.id
        )
        ...
        if user_crafting["end_date"] == None:
            if await can_craft(inv=inv, recipe=craftable_items[self.selected_item]):
                ...
                await data_manager.edit_user_data(
                    interaction.user.id, "inventory", json.dumps(inv)
                )
                minutes = items[self.selected_item]["crafting_time"]

                await data_manager.add_crafting(
                    interaction.user.id, self.selected_item, minutes
                )
            ...
            elif not await can_craft(
                inv=inv, recipe=craftable_items[self.selected_item]
            ):
                return await interaction.followup.send(
                    content=f"You do not have the items to craft this item",
                    ephemeral=True,
                )

        elif user_crafting["end_date"] > now:
            item = user_crafting["item"]
            end_date = discord.utils.format_dt(
                datetime.datetime.fromisoformat(user_crafting["end_date"]), style="R"
            )
            return await interaction.followup.send(
        ...
        async with data_manager.db_connection.acquire():
            ...
            for craft in ended_craftings:
                ...
                await data_manager.edit_user_inventory(
                    craft["user_id"], user_crafting["item"], 1
                )
