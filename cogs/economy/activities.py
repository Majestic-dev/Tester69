...
class activities(commands.Cog):
    ...
    async def hunt(self, interaction: discord.Interaction):
        ...
        if await cooldown_check(
            ...
            chances = []
            for item in data_manager.get("economy", "hunting items").values():
    ...
    async def fish(self, interaction: discord.Interaction):
        ...
        if await cooldown_check(
            ...
            chances = []
            for item in data_manager.get("economy", "fishing items").values():
                chances.append(item["chance"])
            items = list(data_manager.get("economy", "fishing items").keys())
            item_name = random.choices(items, weights=chances, k=1)
            item_emoji = data_manager.get("economy", "items")[item_name[0]]["emoji"]
