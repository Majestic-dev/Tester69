...
class forgingview(discord.ui.View):
    ...
    @discord.ui.button(
        label="Forge", emoji="🔥", style=discord.ButtonStyle.green, row=1
    )
    async def forge_callback(
        ...
        user_forging: ForgingData = await data_manager.get_user_forging(
            interaction.user.id
        )
        ...
        if user_forging["end_date"] == None:
            if await can_forge(inv=inv, recipe=forgable_items[self.selected_item]):
                ...
                await data_manager.add_forging(
            ...
            elif not await can_forge(
                inv=inv, recipe=forgable_items[self.selected_item]
            ):
            ...
            end_date = discord.utils.format_dt(
                datetime.datetime.fromisoformat(user_forging["end_date"]), style="R"
            )
            return await interaction.followup.send(
        ...
        async with data_manager.db_connection.acquire():
            ...
            for forge in ended_forgings:
                ...
                await data_manager.edit_user_inventory(
                    forge["user_id"], user_forging["item"], 1
                )
