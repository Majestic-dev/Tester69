...
from utils import data_manager, GuildData, FilteredWordsData
...
class logging_listeners(commands.Cog):
    ...
    async def on_warning(
        ...
        guild_data: GuildData = await data_manager.get_guild_data(guild.id)
        logs_channel = self.bot.get_channel(guild_data["logs_channel_id"])
    ...
    async def on_member_update(self, before, after):
        guild_data: GuildData = await data_manager.get_guild_data(before.guild.id)
        logs_channel = self.bot.get_channel(guild_data["logs_channel_id"])
    ...
    async def on_guild_role_create(self, role):
        guild_data: GuildData = await data_manager.get_guild_data(role.guild.id)
        logs_channel = self.bot.get_channel(guild_data["logs_channel_id"])
    ...
    async def on_guild_channel_create(self, channel):
        guild_data: GuildData = await data_manager.get_guild_data(channel.guild.id)
        logs_channel = self.bot.get_channel(guild_data["logs_channel_id"])
