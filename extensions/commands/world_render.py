...
class world_render(commands.Cog):
    ...
    async def load_render(self, world: str, member_id: int):
        ...
        response = await session.get(f"http://world.growtopiagame.com/{world}.png")
    ...
    @app_commands.command(
        name="gt_world",
        description="Get a world render from growtopia (if the world is rendered)",
    )
    ...
    async def world_gt(self, interaction: discord.Interaction, world: str):
        ...
        embed.add_field(
            name="",
            value=f"**World | [{world.upper()}](http://world.growtopiagame.com/{world}.png)**",
            inline=False,
        )
        ...
        await interaction.edit_original_response(
            embed=embed, attachments=[discord.File(f"{interaction.user.id}.png")]
        )
        os.remove(f"{interaction.user.id}.png")
